...
//create a function initSettingClickHandlers, which initiates 2 functions : applyClickToCharImg,applyClickToStart;
function initSettingClickHandlers(){
    applyClickToCharImg();
    applyClickToStart();
}
//create a function applyClickToCharImg, which applies click handlers to .charImg, when clicked remove current 'selected' class
//from its one of the siblings and add the class to the clicked element
//do it for both 1p and 2p.
function applyClickToCharImg(){
    $('.characters').on('click','.charImg',function(){
        $(this).siblings().removeClass('selected');
        $(this).addClass('selected');
    })
}
//create a function applyClickToStart, which adds a click handler to the start button, that gathers information that will be used to call func. initGame, calls initGame
//calls removeClickOnSetting;
function applyClickToStart(){
    $('#settingsButtons').on('click','#start',function(){
        var numberOfPlayers = $('.player-buttons:checked').val();
        var gameBoardSize = $('.game-size:checked').val();
        var p1Img = $('.player1Char > .selected').css('background-image');
        //        var p1Image = $('.player1Char > .selected').css('background-image');
        var p2Img = $('.player2Char > .selected').css('background-image');
        console.log(p1Img);
        console.log(p2Img);
        $('.gameBoard *').remove();
        initGame(+numberOfPlayers,+gameBoardSize,p1Img,p2Img);
        removeClickOnSetting();
    });
}
//create a function removeClickOnSetting which removes current click handlers in setting section
function removeClickOnSetting(){
